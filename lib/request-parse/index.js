...
module.exports = function (req) {
  ...
  let properties = (type == 'mysql' || type == 'pg') ? database(system, qs, req.method, object, type) : (type == 'web') ? web(system, qs) : undefined;
...
function database(system, qs, http_method, object, type) {
  ...
  if (!systems[system] || !db_method || !name) {
    return;
  }

  let sqlStr;
  if (db_method == 'procedure') {
    sqlStr = sql.get('SELECT ' + name + '()', querystring.parse(qs), type, db_method);
    if (type == 'mysql') sqlStr = sql.get('CALL ' + name + '()', querystring.parse(qs), type, db_method);
  } else if (db_method == 'function') {
    sqlStr = sql.get('SELECT ' + name + '()', querystring.parse(qs), type, db_method);
  } else {
    switch (http_method) {
    case 'GET':
    ...
    case 'DELETE':
      ...
      break;
    case 'POST':
      sqlStr = sql.get('INSERT INTO ' + name + ' VALUES ?', [], type);
      break;
    }
  }
