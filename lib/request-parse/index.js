...
function database(system, qs, http_method, object) {
  ...
  if (db_method == 'procedure') {
    ...
    switch (http_method) {
    case 'GET':
      sqlStr = sql.get('SELECT * FROM ??', querystring.parse(qs));
      break;
    case 'PATCH':
      sqlStr = sql.get('UPDATE ?? SET ?', querystring.parse(qs));
      break;
    case 'DELETE':
      sqlStr = sql.get('DELETE FROM ??', querystring.parse(qs));
      break;
    case 'POST':
