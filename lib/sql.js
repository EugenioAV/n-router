exports.get = function (method, obj, getFormatted, n) {  
  let query = { sql: method, value: [] };
  const keys = Object.keys(obj);

  query = formationOfWhere(query, obj, keys, getFormatted, n);
...
function formationOfWhere(query, obj, keys, getFormatted, n) {
  let amp = ' WHERE ';
  let num = 0;
  ...
  for (let i=0; i < keys.length; i++) {
    ...
    if (typeof value === 'object' && keys[i] != 'null') {
      ...
      for (let k=0; k < deepKeys.length; k ++) {
        ...
        if (!sign) continue;
        query.sql += amp + keys[i] + sign + getFormatted(++num + n);
        query.value.push(getValue(value[deepKey]));
        amp = ' && ';
      }
    } else if (keys[i] != 'null'){
      query.sql += amp + keys[i] + ' = ' + getFormatted(++num + n);
