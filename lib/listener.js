...
function listener(req, res) {
  ...
  const { type, system, qs, name, format, accept_type, convert_type } = requestParse(req);
  ...
  const adapter = pool[system] || new Adapter(path, convert_type, accept_type);
  ...
  adapter[action]({type, name, qs}, (err, stream) => {
    ...
    result.on('error', (err) => handleError(err, res));
    transformer.on('error', (err) => handleError(err, res));
  });
}
